...
const makeAuthenticationV1Handlers = (dependencies: HandlerDependencies): AuthenticationHandler => ({
  authenticate: async (ctx: Context) => {
    const authRequest = urlQueryToApiQuery<AuthenticationApiRequest>(ctx.request.query)

    const { error: validationError, value: validRequest } = validateAuthenticationRequest(authRequest)
  ...
  logged: async (ctx: Context) => {
    ...
    const response = await dependencies.authenticationService.getToken(code as string, ctx)
  }
})
